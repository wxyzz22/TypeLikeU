...
def processing_folder(folder_path, sample_size, train_size):
    '''
    Function that takes in the folder where all `.txt` files of users' keystroke data are stored and generates pandas dataframes, along
    the way, split the dataframe into train-dev-test sets according to the specified sizes. Moreover, the output also includes one-hot encoders
    for the keycodes.
    Input:
        folder_path
        sample_size: number of users to use as the full dataset (including train, dev, test sets)
        train_size: the percentage of samples to be used as trainset
        dev_size: the percentage of samples to be used as devset
    Output:
        data, train_data, dev_data, test_data
        KEYCODE_enc: onehot encoder for unigraph ['KEYCODE'] column (fit on train_data)
        K1_K2_enc: onehot encoder for digraph ['K1', 'K2'] column groups (fit on train_data)
        K1_enc, K2_enc: onehot encoder for digraph ['K1', 'K2'] columns individually (fit on train_data)
    '''
